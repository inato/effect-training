...
test("should return true if the provided number is odd", () => {
  const oddValue = 1;

  expect(isOddP(oddValue)).toBe(true);
});

test("should return false if the provided number is even", () => {
  const oddValue = 2;
...
test("should return the correct next element in the Collatz sequence", () => {
  expect(next(4)).toBe(2);
  expect(next(2)).toBe(1);
  expect(next(1)).toBe(4);
});

test("should return the correct element in the Collatz sequence 3 steps ahead", () => {
